...
func (w *TranslateMissingWorker) translateEvent(ctx context.Context, q *db.Queries, e db.ListEventsMissingZhRow) error {
	in := ai.TranslateInput{Names: e.Names}
	if e.TitleZh == "" {
		in.Title = e.Title
	}
	if e.SummaryZh == "" {
		in.Summary = e.Summary
	}
	tr, err := w.deps.AI.Translate(ctx, in)
	if err != nil {
		return err
	}
	if (in.Title != "" && tr.Title == "") || (in.Summary != "" && tr.Summary == "") {
		return errors.New("model returned an empty title")
	}
	return q.SetEventZh(ctx, db.SetEventZhParams{ID: e.ID, TitleZh: tr.Title, SummaryZh: tr.Summary})
}
